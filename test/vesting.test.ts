...
describe('Vesting', () => {
  ...
  beforeEach(async () => {
    testToken = <CustomToken>await deployContract("CustomToken", "Paycer token", "PAYCER", totalSupply);
    vesting = <Vesting>await deployProxy("Vesting", testToken.address, vestingParams);

    await testToken.transfer(vesting.address, totalAmount);
  });
  ...
  describe("initialize", async () => {
    it("Validiation of initilize params", async () => {
      ...
      await expect(deployProxy("Vesting", ethers.constants.AddressZero, params)).to.be.revertedWith("initialize: vestingToken cannot be zero");
      await expect(deployProxy("Vesting", testToken.address, { ...params, releaseRate: 0 })).to.be.revertedWith("initialize: release rate cannot be zero");
      await expect(deployProxy("Vesting", testToken.address, { ...params, releaseInterval: 0 })).to.be.revertedWith("initialize: release interval cannot be zero");
    });
  });

  describe("updateRecipient", async () => {
    it("Only owner or worker can call this function", async () => {
      await expect(vesting.connect(signers[2]).updateRecipient(signers[1].address, "1")).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(vesting.connect(signers[3]).updateRecipient(signers[1].address, "1")).to.be.revertedWith("Ownable: caller is not the owner");
  ...
  describe("vested", () => {
    ...
    it("Should only release every interval", async () => {
      ...
      const initialUnlockAmoumnt = amount.mul(vestingParams.initialUnlock).div(1e10);
      const releaseAmount = amount.mul(vestingParams.releaseRate).div(1e10);

      await setNextBlockTimestamp(lockEndTime + 1);
      await mineBlock();
      expect(await vesting.vested(signers[1].address)).to.be.equal(initialUnlockAmoumnt);

      await setNextBlockTimestamp(lockEndTime + vestingParams.releaseInterval - 1);
      await mineBlock();
      expect(await vesting.vested(signers[1].address)).to.be.equal(initialUnlockAmoumnt);

      await setNextBlockTimestamp(lockEndTime + vestingParams.releaseInterval * 2 - 1);
      await mineBlock();
      expect(await vesting.vested(signers[1].address)).to.be.equal(initialUnlockAmoumnt.add(releaseAmount));

      await setNextBlockTimestamp(lockEndTime + vestingParams.releaseInterval * 3 - 1);
      await mineBlock();
      expect(await vesting.vested(signers[1].address)).to.be.equal(initialUnlockAmoumnt.add(releaseAmount.mul(2)));

      await setNextBlockTimestamp(lockEndTime + vestingParams.releaseInterval * 4 - 1);
      await mineBlock();
      expect(await vesting.vested(signers[1].address)).to.be.equal(initialUnlockAmoumnt.add(releaseAmount.mul(3)));
    });
    ...
    it("Correct amount should be vested during the lockPeriod", async () => {
      ...
      for (let i = 10; ; i += vestingParams.releaseInterval * 1000) {
        ...
        await mineBlock();
        const vestedAmount = BigNumber.from(i).div(vestingParams.releaseInterval).mul(vestingAmount).mul(vestingParams.releaseRate).div(1e10).add(initialUnlockAmoumnt);
  ...
  describe("withdraw", () => {
    it("If zero, nothing happens", async () => {
      ...
      const balance0 = await testToken.balanceOf(signers[1].address);
      const receipt = await (await vesting.connect(signers[1]).withdraw()).wait();
      expect(receipt.events?.length).to.be.equal(0);
      const balance1 = await testToken.balanceOf(signers[1].address);
    ...
    it("Correct amount is withdrawn/event is emitted", async () => {
      ...
      const passedTime = vestingParams.releaseInterval * 10000 + 1;
      const initialUnlockAmoumnt = vestingAmount.mul(vestingParams.initialUnlock).div(1e10);
      const vestedAmount = BigNumber.from(passedTime).div(vestingParams.releaseInterval).mul(vestingAmount).mul(vestingParams.releaseRate).div(1e10).add(initialUnlockAmoumnt);
